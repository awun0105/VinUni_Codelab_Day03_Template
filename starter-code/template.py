...
class ReActAgent:
    ...
    def plan_and_execute_step(
        self, user_input: str, iteration: int
    ) -> Tuple[str, bool]:
        ...
        needs_flight = any(
            k in user_lower for k in ["chuyến bay", "vé", "bay từ", "vé máy bay"]
        )
        needs_weather = any(
            k in user_lower for k in ["thời tiết", "mặc gì", "nhiệt độ", "mưa"]
        )
        ...
        if needs_flight and iteration == 1:
            ...
            destination = (
                "DAD" if "dad" in user_lower or "đà nẵng" in user_lower else "SGN"
            )
            max_price = 5000000
            ...
            if not needs_weather:
                if not obs:
                    ...
                    final_ans = (
                        f"Tìm thấy {len(obs)} chuyến bay từ {origin} đi {destination}:\n"
                        + "\n".join(lines)
                    )
        ...
        elif needs_weather and (
            iteration == 2 or (iteration == 1 and not needs_flight)
        ):
            ...
            flight_obs = next(
                (
                    t["observation"]
                    for t in self.trace
                    if t.get("action", {}).get("name") == "get_flight_info"
                ),
                [],
            )
            weather_obs = next(
                (
                    t["observation"]
                    for t in self.trace
                    if t.get("action", {}).get("name") == "get_weather_forecast"
                ),
                {},
            )
